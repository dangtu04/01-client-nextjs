...
const CartPage = async (props: IProps) => {
  ...
  const res = await sendAuthRequest<IBackendRes<IModelPaginate<ICartItem> & { totalPrice: number }>>({
  ...
  // console.log("CartPage ~ res:", res);

  return (
    <>
      <Cart items={res?.data?.results ?? []} meta={res?.data?.meta} totalPrice={res?.data?.totalPrice ?? 0}/>
    </>
  );
};
